...
{/* 该页面采用了MDX文件，属于Markdown增强版，不喜欢当前样式整个文件都可自由修改样式 */}
        ...
        <div class="space-y-2.5 flex-1">
          {[
            ...
            <div class="flex items-center justify-between gap-2 rounded-lg bg-black/5 dark:bg-white/5 px-3 py-2">
              ...
              <button onclick={`navigator.clipboard.writeText('${item.value.replace(/'/g, "\\'")}');this.querySelector('span').textContent='已复制';setTimeout(()=>this.querySelector('span').textContent='复制',1500)`} class="shrink-0 inline-flex items-center gap-1 px-2 py-1 rounded-md bg-black/10 dark:bg-white/10 text-(--btn-content) text-[0.65rem] hover:opacity-80 transition-opacity cursor-pointer">
        ...
        <div class="space-y-0 flex-1">
          ...
          <div class="flex gap-3.5">
            ...
            <div class="pb-4">
              ...
              <div class="rounded-lg bg-black/5 dark:bg-white/5 p-4 text-[0.7rem] leading-relaxed overflow-x-auto whitespace-pre">{`站点名称：您的站点名称\n站点描述：您的站点描述\n站点链接：您的站点链接\n头像链接：您的站点头像`}</div>
            </div>
          </div>
